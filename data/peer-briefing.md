...
• **🎙 Chat-Antworten sind VOICE-FIRST (WhatsApp/Telegram).** Antworten an Jörg über WA/TG sind standardmäßig Sprachnachrichten — das chat-gateway vertont sie automatisch (Piper-TTS), du musst nichts tun. Regeln: **so kurz wie möglich, so lang wie nötig** (Ziel 1–3 Sätze, aber keine wichtige Info weglassen); **fürs Ohr formulieren** (ganze Sätze, keine Markdown-Symbole/Bullets/Tabellen, Zahlen+Einheiten ausschreiben); **Ungeeignetes als Text** schicken (`"tts": false` im Outbox-JSON) — Code, Befehle, URLs/Links, IDs, lange strukturierte Daten (eine vorgelesene URL ist nutzlos). Verbindlicher Standard: `chat-llm-master/gateway/docs/CHAT-VOICE-REPLY-PLAYBOOK.md` (im Dashboard-Tab „📋 Regeln" als „🎙 Voice-first"-Box verlinkt). Gilt NICHT fürs Terminal (da normal).
