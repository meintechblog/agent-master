...
async function callOllama({ localModel, prompt, system, maxTokens, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const t0 = Date.now();
  ...
  let r;
  try {
    r = await fetch(`${OLLAMA_HOST}/api/generate`, {
      method: "POST",
    ...
    clearTimeout(timer);
    if (e.name === "AbortError") throw new Error(`ollama timeout after ${timeoutMs}ms — is ollama running at ${OLLAMA_HOST}?`);
    throw new Error(`ollama unreachable at ${OLLAMA_HOST}: ${e.message}. Start it with \`ollama serve\` or set OLLAMA_HOST.`);
  }
  clearTimeout(timer);
  if (!r.ok) {
    const body = await r.text();
  ...
  return {
    ...
    json: null,
    model: localModel,
    logical_model: `local:${localModel}`,
    provider: "ollama",
    latency_ms,
    usage: {
...
export async function complete(input) {
  ...
  if (typeof model === "string" && model.startsWith("local:")) {
    const result = await callOllama({
      localModel: model.slice("local:".length),
...
export async function listModels(probeOllama = false) {
  ...
  if (probeOllama) {
    try {
      ...
      if (r.ok) {
        const json = await r.json();
        local = (json.models || []).map((m) => ({
          name: m.name,
          size: m.size,
          modified_at: m.modified_at,
          logical: `local:${m.name}`,
        }));
      }
    } catch { /* ollama not running — empty list */ }
  }
