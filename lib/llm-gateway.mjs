...
export async function complete(input) {
  ...
  if (external) {
    // Reasoning-style backends need a much higher max_tokens than what
    // template defaults set (calibrated for Anthropic). If the caller didn't
    // explicitly raise max_tokens and the template/default is below the
    // backend's minimum, bump it. Caller's explicit value always wins.
    ...
    try {
      ...
      if (template) result.template = template;
      if (expected_format) result.expected_format = expected_format;
      // Shadow cost: what would Anthropic have charged? Lets callers see
      // the value of running on Jonas's hardware as a dollar number.
      ...
      // Provider-down → fall back to Anthropic. Only retry on transient/server
      // errors; caller mistakes (4xx) and config issues throw through.
      if (!isExternalProviderError(err)) throw err;
