...
// Timeout strategy.
//
// fetch() can't directly expose TCP-connect — once it starts, TCP+TLS+
// request-send+headers-recv all happen in one shot. For NON-STREAM
// OpenAI chat-completion calls, the server (LM Studio in klick's case)
// holds the connection open until inference is fully done, so
// headers-arrival ≈ end-of-inference. A 3s "connect timeout" would kill
// healthy slow inferences. So for non-stream calls we use ONE generous
// read-timeout (caller-supplied, default 60s).
//
// For STREAM calls, the server flushes headers immediately and starts
// emitting deltas as it generates — so an aggressive 5s TTFB-timeout
// catches a dead backend without harming a working one.
//
// "Is the backend reachable at all" gets answered by the periodic
// /v1/models heartbeat (3s timeout) — fast endpoint, doesn't run
// inference. Plus the circuit breaker observes call failures and
// short-circuits subsequent traffic.
const STREAM_TTFB_TIMEOUT_MS = 5_000;   // for streaming calls only
 ...
 */
export async function callOpenAICompat({ backendName, backend, modelId, prompt, system, maxTokens, timeoutMs = DEFAULT_READ_TIMEOUT_MS }) {
  ...
  const t0 = Date.now();
  // Non-stream: only a read-timeout. Headers don't arrive until inference is
  // done (LM Studio doesn't flush early in non-stream mode), so any
  // sub-inference connect-timeout would kill healthy slow calls.
  // Backend-down detection in this path relies on:
  //   (a) the discovery heartbeat (fast endpoint, 3s timeout)
  //   (b) the circuit breaker accumulating failures across calls
  // For first-time fast detection of a dead backend, use the stream variant.
  ...
  try {
    ...
    if (ctrl.signal.aborted) throw new Error(`${backendName} read timeout after ${timeoutMs}ms`);
 ...
 */
export async function callOpenAICompatStream({ backendName, backend, modelId, prompt, system, maxTokens, timeoutMs = DEFAULT_READ_TIMEOUT_MS, includeThinking = false }, onEvent) {
  const apiKey = resolveApiKey(backend);
  const url = `${backend.base_url.replace(/\/$/, "")}/v1/chat/completions`;
  const effectiveMax = maxTokens || backend.default_max_tokens || 4096;
  ...
  const t0 = Date.now();
  // Stream: aggressive TTFB-timeout (5s) until headers arrive, then a
  // generous read-timeout for the streaming body. SSE servers flush headers
  // immediately upon accepting the request, so >5s without headers ≈ dead.
  const ctrl = new AbortController();
  let firstByteSeen = false;
  let abortReason = null;
  const ttfbTimer = setTimeout(() => {
    if (!firstByteSeen) { abortReason = "ttfb"; ctrl.abort(); }
  }, STREAM_TTFB_TIMEOUT_MS);
  const readTimer = setTimeout(() => { abortReason = "read"; ctrl.abort(); }, timeoutMs);
  let r;
  try {
    r = await fetch(url, {
      method: "POST",
    ...
    clearTimeout(ttfbTimer);
    clearTimeout(readTimer);
    if (abortReason === "ttfb") throw new Error(`${backendName} TTFB timeout after ${STREAM_TTFB_TIMEOUT_MS}ms — backend likely down`);
    if (abortReason === "read") throw new Error(`${backendName} read timeout after ${timeoutMs}ms`);
    throw new Error(`${backendName} unreachable: ${e.message}`);
  }
  if (!r.ok) {
    clearTimeout(readTimer);
